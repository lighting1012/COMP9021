...
def find_leaves(tre):
	...
	if flag == False:
		global leaves
		leaves = []
	if tre.value is None:
		return leaves
	else:
		if tre.left_node.value is None and tre.right_node.value is None:
			leaves.append(tre.value)
		else:
			if tre.left_node.value is not None:
				flag = True
				find_leaves(tre.left_node)
			if tre.right_node.value is not None:
				flag = True
				find_leaves(tre.right_node)
	return leaves
def max_diff_in_consecutive_leaves(tree):
	leaves = find_leaves(tree)
	difference = []
	if len(leaves) == 0 or len(leaves) == 1:
		return 0
	else:
		for i in range(len(leaves)-1):
			difference.append(leaves[i+1]-leaves[i])
